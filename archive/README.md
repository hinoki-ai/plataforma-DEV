# 📦 Archived Documentation

This directory contains historical documentation that is no longer actively maintained but kept for reference and context.

**Last Updated**: October 13, 2025

---

## 📁 Directory Structure

### `authentication-fixes-2024/`

Historical authentication fix documentation from October 2024:

- **AUTHENTICATION_SUMMARY.md** - October 2024 fix overview
- **AUTHENTICATION_FIXED.md** - Fix implementation details
- **AUTH_SYSTEM_ANALYSIS.md** - Root cause analysis
- **AUTH_TEST_CREDENTIALS.md** - Test credentials used during fix
- **PRODUCTION_DEPLOYMENT.md** - Deployment guide specific to the auth fix

**Why archived**: These issues were resolved and the authentication system has been stable since. Current auth documentation is in `docs/AUTHENTICATION_SYSTEM_DOCS.md`.

---

### `incidents/`

Historical incident reports:

- **INCIDENT_REPORT_AUTH_FIX_2025-09-01.md** - September 2025 authentication incident

**Why archived**: Incident resolved. Kept for historical record and lessons learned.

---

### `deprecated/`

Documentation for deprecated features or outdated implementation guides:

- **DEPLOYMENT_GUIDE.md** - Generic deployment guide (replaced by project-specific DEPLOYMENT.md)
- **PARENT_REGISTRATION_IMPLEMENTATION.md** - Old parent registration implementation
- **INSTITUTION_SUPPORT_UPDATE.md** - Old institution support update
- **I18N_SYSTEM_VERIFICATION.md** - Internationalization verification (feature complete)

**Why archived**: 
- Features have been fully implemented or replaced
- Documentation superseded by current guides
- Technology migration complete (e.g., Prisma → Convex)

---

## 📚 Current Documentation

For up-to-date documentation, see:

- **[../DOCUMENTATION_INDEX.md](../DOCUMENTATION_INDEX.md)** - Complete documentation index
- **[../START_HERE.md](../START_HERE.md)** - Getting started guide
- **[../CLAUDE.md](../CLAUDE.md)** - Development guide
- **[../docs/](../docs/)** - Technical documentation

---

## 🔍 Using Archived Documents

These documents are useful for:

1. **Historical context** - Understanding past issues and solutions
2. **Learning** - Reviewing troubleshooting approaches
3. **Reference** - Comparing old vs current implementations
4. **Compliance** - Maintaining incident history

**Note**: Do not use these documents for current development. Always refer to the current documentation index.

---

## 🗑️ Archival Policy

Documents are archived when:

1. **Issue resolved** - The problem described no longer exists
2. **Feature complete** - Implementation finished and documented elsewhere
3. **Technology deprecated** - Replaced by new stack (e.g., Prisma → Convex)
4. **Duplicate removed** - Consolidated into canonical documentation
5. **Historical value** - Worth keeping for reference but not actively maintained

---

**Maintained by**: Development team  
**Archive retention**: Indefinite (until no longer needed for reference)

