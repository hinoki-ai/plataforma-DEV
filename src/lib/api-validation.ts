...
export async function validateApiRequest<T>(
  ...
  try {
    const { bodySchema, querySchema, requiredRole, requireAuth = true } = options;
    ...
    return {
      success: true,
      data: bodyData as T,
      session: session as NonNullable<Awaited<ReturnType<typeof auth>>>,
    };
...
export const ApiSchemas = {
  ...
  createMeeting: z.object({
    ...
    guardianPhone: z.string().regex(/^\+?[\d\s\-\(\)]+$/, 'Invalid phone format'),
  ...
  createCalendarEvent: z.object({
    ...
    category: z.enum(['ACADEMIC', 'HOLIDAY', 'SPECIAL', 'PARENT', 'ADMINISTRATIVE', 'EXAM', 'MEETING', 'VACATION', 'EVENT', 'DEADLINE', 'OTHER']),
