...
const namespaces = Object.keys(
  translations.es,
) as (keyof typeof translations.es)[];
...
export function validateTranslations(): ValidationResult {
  ...
  for (const namespace of namespaces) {
    ...
    if (missingInEN.length > 0) {
      ...
      errors.push(
        `Missing ${missingInEN.length} keys in ${namespace} (English): ${missingInEN.slice(0, 5).join(", ")}${missingInEN.length > 5 ? "..." : ""}`,
      );
    }
    ...
    if (extraInEN.length > 0) {
      ...
      warnings.push(
        `Extra ${extraInEN.length} keys in ${namespace} (English): ${extraInEN.slice(0, 3).join(", ")}${extraInEN.length > 3 ? "..." : ""}`,
      );
    }
    ...
    if (missingInES.length > 0) {
      ...
      errors.push(
        `Missing ${missingInES.length} keys in ${namespace} (Spanish): ${missingInES.slice(0, 5).join(", ")}${missingInES.length > 5 ? "..." : ""}`,
      );
    }
    ...
    if (extraInES.length > 0) {
      ...
      warnings.push(
        `Extra ${extraInES.length} keys in ${namespace} (Spanish): ${extraInES.slice(0, 3).join(", ")}${extraInES.length > 3 ? "..." : ""}`,
      );
    }
  }
...
export function getTranslationStats(): TranslationStats {
  ...
  const consistencyScore = Math.max(
    0,
    Math.min(100, ((totalKeys - missingTranslations) / totalKeys) * 100),
  );
 ...
 */
export function searchTranslationKeys(
  searchTerm: string,
...
export function getNamespaceKeys(
  namespace: keyof typeof translations.es,
  language: Language = "es",
): string[] {
  const namespaceTranslations = translations[language][namespace];
...
export function keyExists(
  key: string,
  namespace: keyof typeof translations.es = "common",
  language?: Language,
): boolean {
  if (language) {
    const namespaceTranslations = translations[language][namespace];
