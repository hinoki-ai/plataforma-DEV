  ...
  return (
    <Card className="border-blue-200 dark:border-blue-800">
      <CardHeader>
        ...
        <CardDescription>Overall status of all system components</CardDescription>
      </CardHeader>
          ...
          {Object.entries(healthMetrics).map(([key, value]) => (
            <div key={key} className="space-y-2">
              ...
              <Progress
                ...
                className="h-2"
                indicatorClassName={
                  value >= 98 ? 'bg-green-500' :
                  value >= 95 ? 'bg-yellow-500' : 'bg-red-500'
                }
              />
            </div>
          ))}
...
function SecurityAlertsCard() {
  const alerts = useMemo(() => [
    { id: 1, type: 'warning', message: 'Unauthorized access attempt detected', time: '2 min ago', severity: 'medium' },
    { id: 2, type: 'info', message: 'Automatic backup completed', time: '15 min ago', severity: 'low' },
    { id: 3, type: 'success', message: 'Security update applied', time: '1 hour ago', severity: 'low' },
  ], []);
...
export function MasterDashboard() {
  ...
  const quickActions = masterQuickActions.map(action => ({
    ...
    variant: action.variant as 'default' | 'destructive' | 'outline' | 'secondary',
    href: action.href,
  }));
  ...
  const stats = [
    ...
    {
      icon: Zap,
      value: ((dashboardStats.performance?.throughput || 0) / 1000).toFixed(1) + 'K',
      label: 'Requests/hora',
