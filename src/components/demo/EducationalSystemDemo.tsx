...
export function EducationalSystemDemo() {
  const [selectedType, setSelectedType] = useState<EducationalInstitutionType>('PRESCHOOL');
  const [activeDemo, setActiveDemo] = useState<'overview' | 'navigation' | 'dashboard'>('overview');

  const getInstitutionIcon = (type: EducationalInstitutionType) => {
    switch (type) {
      case 'PRESCHOOL': return <School className="h-5 w-5" />;
      case 'BASIC_SCHOOL': return <BookOpen className="h-5 w-5" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
        <CardHeader>
          ...
          <CardDescription className="text-blue-100">
            Sistema adaptable desde Educación Parvularia hasta Educación Superior
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Institución seleccionada:</strong> {INSTITUTION_TYPE_INFO[selectedType].chileanName} • 
          <strong> Descripción:</strong> {INSTITUTION_TYPE_INFO[selectedType].description} • 
          <strong> Niveles:</strong> {INSTITUTION_TYPE_INFO[selectedType].levels.length} • 
          <strong> Asignaturas:</strong> {getSubjectsForInstitutionType(selectedType).length}
        </AlertDescription>
      </Alert>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  {INSTITUTION_TYPE_INFO[selectedType].levels.map((level) => (
                    <div key={level.id} className="flex items-center justify-between p-3 border rounded">
                      <div>
                        ...
                        <div className="text-xs text-gray-500">{level.ages} • ISCED {level.isced}</div>
                      </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <h4 className="font-medium mb-2">Asignaturas ({getSubjectsForInstitutionType(selectedType).length})</h4>
                  <div className="flex flex-wrap gap-1">
                    {getSubjectsForInstitutionType(selectedType).slice(0, 6).map((subject) => (
                    ...
                    {getSubjectsForInstitutionType(selectedType).length > 6 && (
                      <Badge variant="outline" className="text-xs">
                        +{getSubjectsForInstitutionType(selectedType).length - 6} más
                      </Badge>
                    )}
                  </div>
                </div>
      ...
      <Card className="border-green-200 bg-green-50">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Sistema Educativo', status: 'Completado', color: 'bg-green-500' },
              { label: 'Niveles ISCED', status: 'Completado', color: 'bg-green-500' },
              { label: 'Base de Datos', status: 'Completado', color: 'bg-green-500' },
              { label: 'Interface Admin', status: 'Completado', color: 'bg-green-500' },
              { label: 'Navegación Adaptiva', status: 'Completado', color: 'bg-green-500' },
              { label: 'Dashboard Específico', status: 'Completado', color: 'bg-green-500' },
              { label: 'APIs y Hooks', status: 'Completado', color: 'bg-green-500' },
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded border border-blue-200">
            <div className="text-sm text-blue-800">
              <strong>🎉 Sistema Completado:</strong> El sistema educativo comprensivo está listo para usar. 
              Soporta desde Educación Parvularia hasta Educación Superior con estándares chilenos e internacionales ISCED.
            </div>
          </div>
        </CardContent>
      </Card>
