...
import { AlertTriangle, RefreshCw, Home, Bug, Zap, Wifi, WifiOff } from 'lucide-react';
...
const getErrorMessage = (errorType: string, isOnline: boolean, t: any) => {
  switch (errorType) {
    case 'network':
      return isOnline
        ? t('error.network_error', 'Error de conexión. Verifica tu conexión a internet.')
        : t('error.offline_message', 'No tienes conexión a internet. Algunas funciones pueden no estar disponibles.');
    case 'database':
      return t('error.search_problem', 'Tuvimos un problema al buscar la información. Ya estamos trabajando para solucionarlo.');
    case 'auth':
      return t('error.session_expired', 'Tu sesión terminó o no tienes permiso para entrar aquí. Vuelve a iniciar sesión si es necesario.');
    case 'validation':
      return t('error.general', 'Los datos proporcionados no son válidos. Por favor, verifica e intenta nuevamente.');
    case 'client':
      return t('error.general', 'Ha ocurrido un error en la aplicación. Estamos trabajando para solucionarlo.');
    case 'server':
      return t('error.general', 'El servidor está experimentando dificultades. Por favor, intenta más tarde.');
    default:
      return t('error.general', 'Algo no funcionó como esperábamos. Intenta nuevamente o cuéntanos si el problema sigue.');
  }
};
  ...
  render() {
    if (this.state.hasError) {
      ...
      return <UnifiedErrorUI
        ...
        showDetails={this.props.showDetails !== false && process.env.NODE_ENV === 'development'}
...
class UnifiedErrorUI extends React.Component<{
  ...
  render() {
    ...
    if (variant === 'minimal') {
      return (
        <div className="flex flex-col items-center justify-center p-4 text-center">
          <div className={`w-8 h-8 bg-gradient-to-br ${theme.bgGradient} rounded-full flex items-center justify-center mb-2`}>
    ...
    if (variant === 'compact') {
      return (
        <Card className={`shadow-lg backdrop-blur-xl bg-white/95 dark:bg-gray-900/95 border-2 ${theme.borderColor}`}>
          <CardContent className="text-center p-6">
            {/* Icon */}
            <div className={`w-12 h-12 bg-gradient-to-br ${theme.bgGradient} rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg`}>
              <IconComponent className={`w-6 h-6 ${theme.iconColor}`} />
            </div>

            {/* Title */}
            <h3 className={`text-lg font-bold bg-gradient-to-r ${theme.gradient} bg-clip-text text-transparent mb-2`}>
              {errorTitle}
            </h3>
            ...
            <div className="flex gap-2 justify-center">
              {enableRetry && retryCount < 3 && (
                <Button onClick={onRetry} size="sm" className={`bg-gradient-to-r ${theme.gradient} hover:opacity-90`}>
            ...
            {showDetails && (
              <details className="mt-4 text-left">
                ...
                <div className="mt-2 space-y-1 text-xs">
                  <div><strong>ID:</strong> <code className="text-xs">{errorId}</code></div>
                  <div><strong>Message:</strong> <code className="text-red-600 dark:text-red-400">{error.message}</code></div>
                  {error.stack && <div><strong>Stack:</strong> <pre className="text-xs overflow-auto max-h-20 bg-gray-100 dark:bg-gray-800 p-1 rounded mt-1">{error.stack.slice(0, 400)}...</pre></div>}
                  {context && <div><strong>Context:</strong> {context}</div>}
                  <div><strong>Network:</strong> 🟢 Online | Retries: {retryCount}</div>
                </div>
              </details>
            )}
          </CardContent>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-background">
        <Card className={`shadow-2xl backdrop-blur-xl bg-white/95 dark:bg-gray-900/95 border-2 ${theme.borderColor} max-w-2xl w-full`}>
          <CardHeader className="text-center pb-4">
            {/* Icon */}
            <div className={`w-20 h-20 bg-gradient-to-br ${theme.bgGradient} rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl`}>
              <IconComponent className={`w-10 h-10 ${theme.iconColor}`} />
            </div>

            {/* Title */}
            <CardTitle className={`text-2xl font-bold bg-gradient-to-r ${theme.gradient} bg-clip-text text-transparent mb-2`}>
              {errorTitle}
            </CardTitle>
          ...
          <CardContent className="text-center space-y-6">
            ...
            <div className="flex gap-3 justify-center">
              {enableRetry && retryCount < 3 && (
                <Button onClick={onRetry} className={`bg-gradient-to-r ${theme.gradient} hover:opacity-90`}>
              ...
              <details className="text-left bg-gray-50 dark:bg-gray-800/50 rounded-xl p-5 border border-gray-200 dark:border-gray-700">
                ...
                <div className="mt-3 space-y-3">
                  <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                    <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Error ID</div>
                    <div className="font-mono text-sm text-gray-800 dark:text-gray-200 break-all">{errorId}</div>
                  </div>
                  <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                    <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Message</div>
                    <div className="font-mono text-sm text-red-600 dark:text-red-400 break-words">{error.message}</div>
                  </div>
                  {error.stack && (
                    <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                      <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Stack Trace</div>
                  ...
                  {context && (
                    <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                      <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Context</div>
                      <div className="text-sm text-gray-800 dark:text-gray-200">{context}</div>
                    </div>
                  )}
                  <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                    <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Network Status</div>
                    <div className="flex items-center space-x-4 text-sm">
                      <span className="text-green-600">🟢 Online</span>
                      <span className="text-gray-600">Retries: {retryCount}</span>
                    </div>
                  </div>
                </div>
              </details>
