...
export function detectErrorSeverity(
  error: any,
  type: ErrorType,
): ErrorSeverity {
  const status = error?.status || error?.statusCode;
...
export function UnifiedErrorHandler({
  ...
  const processedErrorInfo = useMemo(() => {
    ...
    const autoSeverity = error
      ? detectErrorSeverity(error, autoType)
      : "medium";
    ...
    return {
      ...
      details:
        errorInfo?.details ||
        (error?.stack ? error.stack.substring(0, 200) : undefined),
  ...
  return (
    <Card className={cn(config.borderColor, "border", className)}>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              {processedErrorInfo.severity && (
                <Badge
                  variant={getSeverityVariant(processedErrorInfo.severity)}
                >
      ...
      <CardContent className="pt-0">
        ...
        {/* Technical details */}
        {processedErrorInfo.showDetails &&
          (processedErrorInfo.details || processedErrorInfo.context) && (
            <details className="mt-4">
              ...
              <div className="text-xs bg-muted p-3 rounded space-y-1">
                {processedErrorInfo.context && (
                  <div>
                    <strong>Contexto:</strong> {processedErrorInfo.context}
                  </div>
                )}
                {processedErrorInfo.details && (
                  <div>
                    <strong>Detalles:</strong>{" "}
                    <pre className="inline">{processedErrorInfo.details}</pre>
                  </div>
                )}
                {processedErrorInfo.timestamp && (
                  <div>
                    <strong>Ocurrió:</strong>{" "}
                    {processedErrorInfo.timestamp.toLocaleString()}
                  </div>
                )}
              </div>
            </details>
