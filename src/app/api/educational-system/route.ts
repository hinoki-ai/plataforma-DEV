  ...
  try {
    ...
    if (!['ADMIN', 'MASTER'].includes(session.user.role)) {
      return NextResponse.json(
        {
          success: false,
          error: 'Insufficient permissions. Only administrators can modify institution configuration.',
        },
        { status: 403 }
      );
    }
    ...
    if (!institutionType || !['PRESCHOOL', 'BASIC_SCHOOL', 'HIGH_SCHOOL', 'COLLEGE'].includes(institutionType)) {
      return NextResponse.json(
        {
          success: false,
          error: 'Invalid institution type. Must be one of: PRESCHOOL, BASIC_SCHOOL, HIGH_SCHOOL, COLLEGE',
        },
        { status: 400 }
      );
    }
