...
export async function GET(request: NextRequest) {
  try {
    ...
    if (checkRateLimit(request, RATE_LIMITS.API.limit, RATE_LIMITS.API.windowMs, 'calendar')) {
      return NextResponse.json(
        { success: false, error: 'Too many calendar requests. Please try again later.' },
        {
          status: 429,
          headers: getRateLimitHeaders(request, RATE_LIMITS.API.limit, RATE_LIMITS.API.windowMs, 'calendar')
        }
      );
    }
