...
export async function GET(request: NextRequest) {
  try {
    ...
    // Resolve Convex user - session.data?.user.id might be a Clerk ID string
    let user = null;

    // Check if session.data?.user.id is a valid Convex ID (starts with a letter)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    let convexUserId = session.data?.user.id as any;
