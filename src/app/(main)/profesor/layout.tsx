...
export default async function ProfesorLayout({
  ...
  console.log('ProfesorLayout - User:', session.user?.email, 'Role:', session.user?.role);

  // Ensure user has a valid role
  if (!session.user?.role) {
    console.error('ProfesorLayout - No user role found, redirecting to unauthorized');
  ...
  if (!roleAccess.canAccessProfesor) {
    console.log('ProfesorLayout - User does not have profesor access, role:', session.user.role);
    redirect('/unauthorized');
  }
