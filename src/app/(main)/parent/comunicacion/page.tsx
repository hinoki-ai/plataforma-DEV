...
export default function ComunicacionPage() {
  ...
  const fetchCommunications = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setCommunications([...fallbackMessages, ...fallbackNotifications.map(n => ({
  ...
  const notificationsData = communications.filter(comm => comm.type === 'notification');
  ...
  const fallbackMessages: Communication[] = [
    {
      ...
      content: t('parent.communication.sample.preview_meeting') + ' Les esperamos puntualmente.',
    ...
    },
    {
      ...
      content: t('parent.communication.sample.preview_report') + ' El rendimiento ha sido consistente durante el trimestre.',
    ...
    },
    {
      ...
      content: t('parent.communication.sample.preview_appointment') + ' Por favor confirme asistencia.',
  ...
  const fallbackNotifications = [
    {
      ...
      description: t('parent.communication.notifications.school_day.description'),
      type: 'event',
      date: '2024-03-25',
    },
    {
      ...
      description: t('parent.communication.notifications.class_suspension.description'),
      type: 'important',
      date: '2024-03-28',
    },
    {
      ...
      description: t('parent.communication.notifications.uniform_delivery.description'),
      type: 'info',
      date: '2024-03-20',
    },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                {loading ? (
                  <div className="text-center py-4">
                    <div className="animate-pulse">{t('parent.communication.loading_messages')}</div>
                  </div>
                  ...
                  >
                    ...
                    <p className="text-sm text-muted-foreground mb-3">
                      {(message as any).preview || message.content.substring(0, 100) + '...'}
                    </p>
                    <Button
                      variant="outline"
                      size="sm"
                  ...
                  <div className="text-center py-8">
                    <div className="text-muted-foreground">{t('parent.communication.no_messages')}</div>
                  </div>
                )}
              </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {loading ? (
                  <div className="text-center py-4">
                    <div className="animate-pulse">{t('parent.communication.loading_notifications')}</div>
                  </div>
                  ...
                  <div className="text-center py-8">
                    <div className="text-muted-foreground">{t('parent.communication.no_notifications')}</div>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div>
                <h5 className="font-medium text-sm">{t('parent.communication.contact.secretary.title')}</h5>
                <p className="text-xs text-gray-600">
                  {t('parent.communication.contact.secretary.schedule')}
                  <br />
                  {t('parent.communication.contact.secretary.phone')}: {t('parent.communication.contact.secretary.phone_number')}
                </p>
              </div>
              <div>
                <h5 className="font-medium text-sm">{t('parent.communication.contact.director.title')}</h5>
                <p className="text-xs text-gray-600">
                  {t('parent.communication.contact.director.schedule')}
                  <br />
                  {t('parent.communication.contact.director.email')}: {t('parent.communication.contact.director.email_address')}
                </p>
              </div>
              <div>
                <h5 className="font-medium text-sm">{t('parent.communication.contact.emergency.title')}</h5>
                <p className="text-xs text-gray-600">
                  {t('parent.communication.contact.emergency.description')}
                  <br />
                  {t('parent.communication.contact.emergency.phone')}: {t('parent.communication.contact.emergency.phone_number')}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="flex items-center gap-4">
              ...
              <span>Fecha: {selectedMessage ? new Date(selectedMessage.date).toLocaleDateString() : ''}</span>
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            >
              <Link href={`/parent/comunicacion/nuevo?reply=${selectedMessage?.id}`}>
                Responder
              </Link>
            </Button>
          </div>
          ...
          <div className="mt-4 space-y-4">
            {messages.length > 0 ? messages.map(message => (
              ...
              >
                ...
                <p className="text-sm text-muted-foreground mb-3">
                  {(message as any).preview || message.content.substring(0, 150) + '...'}
                </p>
                <Button
                  variant="outline"
                  size="sm"
