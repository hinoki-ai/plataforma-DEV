...
function SubmitButton({ isLoading: parentLoading }: { isLoading: boolean }) {
  ...
  const [optimisticState, setOptimisticState] = useState<'idle' | 'submitting' | 'success'>('idle');
  const { t } = useLanguage();
  ...
  const isLoading = pending || optimisticState === 'submitting' || parentLoading;

  return (
    <Button
      type="submit"
      disabled={isLoading}
      className={cn('w-[90%] transition-all duration-200 bg-[#f88379] hover:bg-[#f88379]/90 text-black', isLoading && 'opacity-90 scale-[0.98]')}
    >
      {isLoading ? (
...
export default function LoginPage() {
  ...
  const redirectUrl = useMemo(() => {
    ...
    switch (session.user.role) {
      ...
      case 'PARENT':
        return '/parent';
      case 'PUBLIC':
        return session.user.needsRegistration ? '/centro-consejo' : '/centro-consejo/dashboard';
      default:
        return '/';
    }
  ...
  const emailError = useMemo(() => (emailTouched && !EMAIL_REGEX.test(email) ? t('auth.validation.email') : ''), [emailTouched, email, t]);

  const passwordError = useMemo(() => (passwordTouched && password.length < 6 ? t('auth.validation.password') : ''), [passwordTouched, password, t]);
  ...
  return (
    <div className="backdrop-blur-xl bg-gray-900/80 border border-gray-700/50 rounded-lg shadow-lg p-6 w-full max-w-sm mx-auto flex flex-col items-center justify-center">
      <h2 className="text-2xl font-semibold text-center text-foreground mb-6">{t('auth.access.title')}</h2>
      <form
        action={dispatch}
        className="w-full"
        autoComplete="off"
      ...
      >
        <div className="space-y-2 mb-4">
          ...
          />
          {emailError && (
            <div id="email-error" className="text-destructive text-xs mt-1" role="alert">
              {emailError}
            </div>
          )}
        </div>
          ...
          <div className="relative">
            ...
            >
              {showPassword ? (
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0zm2.21-2.21A9.956 9.956 0 0122 9c0 5.523-4.477 10-10 10a9.956 9.956 0 01-4.575-1.125M3.27 3.27l17.46 17.46" />
                </svg>
              )}
            </button>
          </div>
          {passwordError && (
            <div id="password-error" className="text-destructive text-xs mt-1" role="alert">
              {passwordError}
            </div>
          )}
        </div>
        ...
        {errorMessage && (
          <div className="text-destructive text-sm text-center mb-6" role="alert">
            {errorMessage}
          </div>
        )}
