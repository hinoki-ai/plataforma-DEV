...
export const getGlobalNodes = query({
  ...
  handler: async (ctx) => {
    ...
    const nodes = institutions.map((inst, index) => {
      ...
      const load = Math.min(
        95,
        Math.floor((userCount / 10) * 20) + Math.floor(Math.random() * 30),
      );
      const latency = Math.floor(Math.random() * 50) + 10;

      // Determine status based on activity
      const status: "online" | "degraded" | "offline" =
        inst.isActive === false ? "offline" : load > 80 ? "degraded" : "online";
...
export const getGlobalAlerts = query({
  ...
  handler: async (ctx) => {
    ...
    const inactiveInstitutions = institutions.filter(
      (inst) => inst.isActive === false,
    );
    ...
    const lowUserInstitutions = institutions.filter((inst) => {
      const userCount = memberships.filter(
        (m) => m.institutionId === inst._id && m.status === "ACTIVE",
      ).length;
...
export const getNetworkTopology = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      activeInstitutions: institutions.filter((i) => i.isActive !== false)
        .length,
      totalUsers: memberships.filter((m) => m.status === "ACTIVE").length,
      regionalLatency,
      connectivity: "100%", // Mock connectivity
      averageLatency: Math.floor(
        Object.values(regionalLatency).reduce((a, b) => a + b, 0) /
          Object.values(regionalLatency).length,
      ),
    };
  },
});
...
function calculateDataTransferred(
