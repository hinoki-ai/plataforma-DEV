...
/**
 * Class Attendance Table - Decreto 67 & Circular N°30 Compliance
 *
 * Records daily student attendance in classes as required by Chilean educational regulations.
 * Supports various attendance statuses including present, absent, late, and justified absences.
 *
 * Relationships:
 * - Belongs to: institutionInfo (1:1)
 * - Belongs to: courses (1:1)
 * - Belongs to: students (1:1)
 * - Belongs to: users (registeredBy - 1:1)
 * - Referenced by: recordCertifications (attendance certification)
 *
 * @compliance Decreto 67 (Attendance Recording), Circular N°30 (Digital Class Book)
 * @statuses Supports 5 attendance statuses: PRESENTE, AUSENTE, ATRASADO, JUSTIFICADO, RETIRADO
 * @audit All attendance records must be digitally signed and certified monthly
 */
/**
 * Class Attendance Records
 *
 * Daily attendance tracking for students in courses.
 * Supports Chilean educational attendance requirements.
 *
 * Relationships:
 * - Belongs to: courses, students (N:1 each)
 * - Belongs to: institutionInfo (N:1)
 * - Belongs to: users (registeredBy)
 *
 * @compliance Circular N°30 (Attendance Record Requirements)
 */
...
/**
 * Learning Objectives (OA) - Decreto 67 Compliance
 *
 * Stores educational learning objectives according to Chilean curriculum standards.
 * Each OA belongs to a specific subject, level, and grade.
 *
 * Relationships:
 * - Has many: evaluationIndicators (1:N)
 * - Referenced by: curriculumCoverage, classContentOA
 *
 * @compliance Decreto 67 (Chilean Educational Standards)
 */
/**
 * Learning Objectives (OA) Table - Decreto 67 Compliance
 *
 * Stores educational learning objectives according to Chilean curriculum standards.
 * Each OA belongs to a specific subject, level, and grade as defined by the Ministry of Education.
 *
 * Relationships:
 * - Belongs to: institutionInfo (1:1)
 * - Has many: evaluationIndicators (1:N evaluation criteria)
 * - Has many: curriculumCoverage (1:N coverage tracking)
 * - Referenced by: classContentOA (daily OA coverage)
 *
 * @compliance Decreto 67 (Chilean Educational Standards), Mineduc Curriculum Frameworks
 * @curriculum Supports OA (Objetivos de Aprendizaje) as defined by national curriculum
 * @structure OA codes follow format: [Subject][Number] (e.g., "OA01", "LEN02")
 */
...
/**
 * Digital Signatures - Circular N°30 Compliance
 *
 * Electronic signatures for educational records and documents.
 * Supports biometric and digital certificate authentication.
 *
 * Relationships:
 * - Belongs to: users (signedBy, certifiedBy)
 * - Belongs to: institutionInfo (N:1)
 *
 * @compliance Circular N°30 (Digital Signature Requirements)
 * @security Cryptographic signature validation required
 */
...
/**
 * Record Certifications - Circular N°30 Compliance
 *
 * Official certification of educational records by authorized personnel.
 * Required for period closures and legal validity.
 *
 * Relationships:
 * - Belongs to: users (certifiedBy)
 * - Belongs to: institutionInfo (N:1)
 *
 * @compliance Circular N°30 (Record Certification Requirements)
 * @audit Certifications must be performed by directors or authorized administrators
 */
export const recordCertifications = defineTable({
  ...
  .index("by_institutionId", ["institutionId"])
  .index("by_recordType_recordId", ["recordType", "recordId"])
  .index("by_certifiedBy_status", ["certifiedBy", "status"])
...
/**
 * Class Content Table - Circular N°30 Compliance (Digital Class Book)
 *
 * Records daily lesson content, objectives, activities, and materials used in classes.
 * Serves as the digital equivalent of the traditional paper-based class book (Libro de Clases).
 *
 * Relationships:
 * - Belongs to: institutionInfo (1:1)
 * - Belongs to: courses (1:1)
 * - Belongs to: users (teacherId - 1:1)
 * - Has many: classContentOA (1:N OA coverage records)
 * - Referenced by: digitalSignatures (signature tracking)
 * - Referenced by: recordLocks (period locking)
 *
 * @compliance Circular N°30 (Digital Class Book Requirements)
 * @audit Requires teacher digital signature and periodic administrative certification
 * @retention Records must be retained for 5 years according to educational regulations
 */
export const classContent = defineTable({
  ...
  .index("by_subject_date", ["subject", "date"])
