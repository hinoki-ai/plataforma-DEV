...
const esCommon = JSON.parse(
  fs.readFileSync("src/locales/es/common.json", "utf8"),
);
const enCommon = JSON.parse(
  fs.readFileSync("src/locales/en/common.json", "utf8"),
);

// Find all Cogníto keys
const cognitoKeys = Object.keys(esCommon).filter((k) =>
  k.startsWith("Cogníto."),
);
...
fs.writeFileSync(
  ...
  JSON.stringify(esCommon, null, 2),
);
fs.writeFileSync(
