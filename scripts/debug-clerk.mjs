...
async function testClerkIntegration() {
  try {
    ...
    console.log(`   CLERK_SECRET_KEY: ${clerkSecret ? "✅ Set" : "❌ Missing"}`);
    console.log(`   NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY: ${clerkPublishable ? "✅ Set" : "❌ Missing"}`);
    console.log(`   CLERK_WEBHOOK_SECRET: ${webhookSecret ? "✅ Set" : "❌ Missing"}`);
    console.log(`   NEXT_PUBLIC_CONVEX_URL: ${convexUrl ? "✅ Set" : "❌ Missing"}`);

    if (!clerkSecret || !clerkPublishable) {
      console.log("\n❌ Missing required Clerk environment variables!");
      console.log("   Please check your .env.local file and ensure Clerk variables are set.");
      return;
    }
    ...
    try {
      ...
      if (usersResponse.data.length > 0) {
        ...
        usersResponse.data.forEach((user, index) => {
          const email = user.emailAddresses.find(e => e.id === user.primaryEmailAddressId)?.emailAddress || 'No email';
      ...
      const roleCounts = { MASTER: 0, ADMIN: 0, PROFESOR: 0, PARENT: 0, PUBLIC: 0 };
    ...
    console.log("   Webhook URL should be: https://plataforma.aramac.dev/api/clerk/webhook");
    console.log(`   Webhook secret: ${webhookSecret ? "✅ Configured" : "❌ Missing"}`);
    console.log("   Events to monitor: user.created, user.updated, user.deleted");
