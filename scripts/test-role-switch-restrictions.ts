...
async function testRoleSwitchAPI(
  ...
  try {
    console.log(
      `🧪 Testing role switch: ${user.role} → ${targetRole} for user ${user.name}`,
    );
    ...
    if (user.role !== "MASTER") {
      console.log(
        `❌ BLOCKED: Only MASTER users can switch roles. User ${user.name} has role ${user.role}`,
      );
      return false;
    }
...
async function runRoleSwitchTests() {
  ...
  masterResults.forEach((result) => {
    console.log(
      `  ${result.allowed ? "✅" : "❌"} ${result.user.name}: ${result.user.role} → ${result.targetRole}`,
    );
  });
  ...
  const allTestsPass =
    masterAllowed && adminBlocked && profesorBlocked && parentBlocked;
