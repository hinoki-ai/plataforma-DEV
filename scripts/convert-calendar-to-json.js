...
const calendarContent = fs.readFileSync(
  "src/data/comprehensive-calendar.ts",
  "utf8",
);

// Extract holidays data
const holidaysMatch = calendarContent.match(
  /export const chileanHolidays: ChileanHoliday\[\] = (\[[\s\S]*?\]);/,
);
if (holidaysMatch) {
...
const academicMatch = calendarContent.match(
  /export const academicCalendar: AcademicEvent\[\] = (\[[\s\S]*?\]);/,
);
if (academicMatch) {
...
const specialMatch = calendarContent.match(
  /export const specialEvents: CalendarEvent\[\] = (\[[\s\S]*?\]);/,
);
if (specialMatch) {
