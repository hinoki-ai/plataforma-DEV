...
async function countUsers() {
  ...
  const showDetails = process.argv.includes('--details') || process.env.DEBUG === 'true';
  ...
  try {
    ...
    if (showDetails) {
      ...
      recentUsers.forEach((user, index) => {
        ...
        console.log(`  ${status} ${user.name} (${user.email}) - ${user.role} [${date}]`);
      });
    ...
    console.error('❌ Failed to count users:', error instanceof Error ? error.message : String(error));
    throw error;
  }
}
