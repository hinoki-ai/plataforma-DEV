...
class AdminLimitsValidator {
  ...
  async validateAll(): Promise<void> {
    console.log('🔍 Starting comprehensive validation of admin limits implementation...\n');
      ...
      const checks = [
        {
          ...
          check: schemaContent.includes('createdByAdmin') && schemaContent.includes('@map("created_by_admin")'),
          message: 'createdByAdmin field should be present in User model with correct mapping'
        },
        {
          name: 'Field type',
        ...
        },
        {
          ...
          check: schemaContent.indexOf('createdByAdmin') > schemaContent.indexOf('isOAuthUser') &&
                 schemaContent.indexOf('createdByAdmin') < schemaContent.indexOf('createdAt'),
          message: 'createdByAdmin should be placed correctly in the User model'
        }
      ];
  ...
  private validateApiImplementation(): ValidationResult {
    try {
      const apiPath = join(this.projectRoot, 'src/app/api/admin/users/route.ts');
      const apiContent = readFileSync(apiPath, 'utf-8');

      const checks = [
        {
          name: 'Admin limit check',
          check: apiContent.includes('adminCount >= 1') && apiContent.includes('Limit to 1 secondary admin'),
          message: 'API should check admin count and limit to 1'
        },
        {
          name: 'Business messaging',
          check: apiContent.includes('support@manitospintadas.com') && apiContent.includes('contacta con soporte'),
        ...
        },
        {
          ...
          check: apiContent.includes('currentAdminsCreated') && apiContent.includes('maxAllowed'),
          message: 'API should return admin limit information'
        },
        {
          name: 'GET endpoint stats',
          check: apiContent.includes('adminLimits') && apiContent.includes('canCreateMoreAdmins'),
  ...
  private validateMigrationScripts(): ValidationResult {
    try {
      const migrationPath = join(this.projectRoot, 'scripts/add-admin-tracking-migration.sql');
      const migrationContent = readFileSync(migrationPath, 'utf-8');

      const checks = [
        {
          name: 'Column creation',
          check: migrationContent.includes('ADD COLUMN "created_by_admin" TEXT'),
          message: 'Migration should add created_by_admin column'
        },
        {
          name: 'Index creation',
          check: migrationContent.includes('CREATE INDEX "users_created_by_admin_idx"'),
          message: 'Migration should create index for performance'
        },
        {
          name: 'Foreign key constraint',
          check: migrationContent.includes('FOREIGN KEY ("created_by_admin") REFERENCES "users"("id")'),
    ...
    try {
      ...
      const checks = [
        ...
        },
        {
          ...
          check: testContent.includes('role: \'ADMIN\'') && testContent.includes('createdByAdmin: testAdmin.id'),
          message: 'Test should verify admin count logic'
        },
        {
          name: 'Cleanup logic',
          check: testContent.includes('deleteMany') && testContent.includes('in: [testAdminEmail'),
          message: 'Test should include proper cleanup'
        },
        {
          name: 'Business logic validation',
          check: testContent.includes('adminCount >= 1') && testContent.includes('Business opportunity'),
      ...
      const checks = [
        {
          ...
          check: apiContent.includes('if (validatedData.role === \'ADMIN\')') &&
                 apiContent.includes('adminCount >= 1'),
          message: 'Business logic should only apply limits to ADMIN role creation'
        },
        {
          name: 'Contact information',
          check: apiContent.includes('support@manitospintadas.com') &&
        ...
        },
        {
          ...
          check: apiContent.includes('error: \'Límite de administradores alcanzado\'') &&
                 apiContent.includes('status: 403'),
          message: 'Business logic should return proper error responses'
        },
        {
          name: 'Limit tracking',
          check: apiContent.includes('currentAdminsCreated') &&
      ...
      const checks = [
        {
          ...
          check: apiContent.includes('Límite de administradores alcanzado') &&
                 apiContent.includes('status: 403'),
          message: 'Should handle admin limit exceeded with proper error'
        },
        {
          name: 'Unauthorized access',
          check: apiContent.includes('session.user.role !== \'ADMIN\'') &&
                 apiContent.includes('status: 401'),
          message: 'Should handle unauthorized access properly'
        },
        {
          name: 'Email conflict',
          check: apiContent.includes('El email ya está registrado') &&
      ...
      const checks = [
        {
          ...
          check: apiContent.includes('adminLimits: {') &&
        ...
        },
        {
          ...
          check: apiContent.includes('temporaryPassword') &&
                 apiContent.includes('adminLimits'),
          message: 'User creation should return both user data and limit information'
        },
        {
          name: 'Consistent structure',
          check: apiContent.includes('canCreateMoreAdmins: adminsCreated < maxAdminsAllowed'),
